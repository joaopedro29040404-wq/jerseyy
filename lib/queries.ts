...
export async function getProducts(){ const s=await createClient(); const {data,error}=await s.from('products').select('*, category:categories(*)').eq('active',true).order('created_at',{ascending:false}); if(error) throw error; return data??[]; }
